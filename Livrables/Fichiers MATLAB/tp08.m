...
while condition > seuil
    ...
    i = 2:Nx-1;
    j = 2:Ny-1;
    V(i,j) = 0.5*((1/dy*dy)*V(i,j+1)+(1/dy*dy)*V(i,j-1)+(1/dx*dx)*V(i+1,j)+(1/dx*dx)*V(i-1,j))*((dx*dx*dy*dy)/(dx*dx+dy*dy));
...
EScalDs = -1/dy*sum(Ey(24,5:39))+1/dy*sum(Ey(30,5:39))-1/dx*sum(Ex(24:30,5))+1/dx*sum(Ex(24:30,39));
Q = EScalDs*8.854e-12;
C = Q/200;
